...
<script type="text/javascript">
    AJS.$(function() {
        //Remove validation error for chains list
        function removeErrorFromField(fieldName) {
            var $form = AJS.$('#createClonedJob');
            var errorSelector = AJS.format('.error[data-field-name="{0}"]', fieldName);
            $form.find(errorSelector).remove();
        }

        AJS.$('#createClonedJob_jobKeyToClone').live('change', function() {
            removeErrorFromField('jobKeyToClone');
        });

        AJS.$('#createClonedJob_chainKeyToClone').live('change', function() {
            removeErrorFromField('chainKeyToClone');
