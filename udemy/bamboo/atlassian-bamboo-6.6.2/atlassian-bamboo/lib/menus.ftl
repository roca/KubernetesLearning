...
[#macro displayWebPanelTabs location params=[] selectedTab=0]
[#assign webItems = action.getWebItemsForSection(location)/]
...
<div class="aui-tabs horizontal-tabs">
    ...
    [#list webItems as item]
        [#if item?index == selectedTab]
            [#assign firstItemClass = "active-tab"]
        [#else]
            [#assign firstItemClass = ""]
        [/#if]
        <li class="menu-item ${firstItemClass}">
            <a href="${item.link.getDisplayableUrl(req)}">${item.webLabel.displayableLabel}</a>
        </li>
    [/#list]
    </ul>
    [#list webItems as item]
        [#if item?index == selectedTab]
            [#assign firstItemClass = "active-pane"]
        [#else]
            [#assign firstItemClass = ""]
        [/#if]
        <div class="tabs-pane ${firstItemClass}" id="${item.link.id}">
            [@ui.renderWebPanels location="${item.key}" params=params/]
        </div>
    [/#list]
</div>
[#else]
    [@ui.renderWebPanels location="${webItems?first.key}" params=params/]
[/#if]
[/#macro]
