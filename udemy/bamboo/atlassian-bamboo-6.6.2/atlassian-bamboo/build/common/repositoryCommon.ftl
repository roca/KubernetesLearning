...
[#macro viewBambooSpecsStatus]
    <h3>[@s.text name="repository.tabs.bamboo.specs.status"/]</h3>
    <p class="description">
        [@s.text name="repository.tabs.bamboo.specs.status.description"]
            [@s.param][@help.href pageKey="specs.troubleshooting"/][/@s.param]
        [/@s.text]
    </p>

    <div class="specs-status-container"></div>
    <script>
        require(['feature/specs-status'], function(SpecsStatus) {
            new SpecsStatus({
                el: '.specs-status-container',
...
[#macro repositorySelector vcsTypeSelectors linkedRepositories repositoryTypeOption selectedRepository="" hasCreateRepoPermission=true isNoneRepositoryAllowed=true]
