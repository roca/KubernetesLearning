...
<head>
    ...
    ${webResourceManager.requireResource('bamboo.web.resources.common:feature-specs-configuration')}
</head>
<body>
...
[#if isEditMode && selectedRepository?has_content]
    ...
    <script>
        require('feature/specs-configuration')({
            ...
            bambooBaseUrl: '${action.baseUrl?js_string}',
            bambooSpecsVersion: '${bambooSpecsVersion?js_string}',
            showWebhookLink: ${action.bambooSpecsDetectionRequiresWebhooks?c},
            webhookSetupDocumentationUrl: '${webhookSetupDocumentationUrl?js_string}',
            showOnboardingDialog: ${action.showOnboardingDialog?c},
            usages: {
                projects: [
    ...
    [#assign bambooSpecsStatusTabContents]
        [@rc.viewBambooSpecsStatus /]
    [/#assign]
    ...
    [#assign bambooSpecsStatusTab={
        'skip': !action.bambooSpecsCapable,
        'id': 'linked-repos-specs-logs-tab',
        'label': action.getText('repository.tabs.bamboo.specs.status'),
        'contents': bambooSpecsStatusTabContents
    } /]
